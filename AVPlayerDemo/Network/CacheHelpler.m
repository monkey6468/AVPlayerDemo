...
@implementation CombineOperation
...
@implementation CacheHelpler

//单例
+ (CacheHelpler *)sharedWebCache {
...
- (CombineOperation *)downloadWithURL:(NSURL *)url
    ...
    __block CombineOperation *operation = [CombineOperation new];
    __weak __typeof(self)wself = self;
    //赋值组合任务WebCombineOperation中的查找缓存NSOperation任务
    operation.cacheOperation = [[CacheHelpler sharedWebCache] queryDataFromMemory:key cacheQueryCompletedBlock:^(NSData *data, BOOL hasCache) {
        //判断是否查找到缓存
        if (hasCache) {
            //查找到则直接返回缓存数据
            if (completedBlock) {
            ...
            operation.downloadOperation = [[DownloadOperation alloc] initWithRequest:request responseBlock:^(NSHTTPURLResponse *response) {
                if (responseBlock) {
                ...
                if (completedBlock) {
                    if (finished && !error) {
                        ...
                        [[CacheHelpler sharedWebCache] storeDataCache:data forKey:key];
                        //任务完成回调
                        completedBlock(data, nil, YES);
                    } else {
                        //任务失败回调
